...
class InsightGenerator:
    """
    Generates LLM insights for anomalies using a configurable LLM backend or Ollama fallback.
    """

    def __init__(self):
        """Initialize with configuration from environment."""
        self.api_key = os.getenv("OPENROUTER_API_KEY")
        self.api_base_url = os.getenv("AGENT_LLM_BASE_URL", "https://openrouter.ai/api/v1")
        self.model = os.getenv("AGENT_LLM_MODEL", os.getenv("OPENROUTER_MODEL", "openrouter/auto"))
        ...
        if self.use_llm:
            print(f"✓ AI Insights: LLM enabled — {self.api_base_url} using {self.model}")
        else:
            print("⚠ AI Insights: API key missing, falling back to templates.")

    async def generate_insight(self, anomaly: Anomaly) -> str:
        """Generate an insight using the best available source."""
        if self.use_llm:
            try:
                return await self._generate_with_llm(anomaly)
            except Exception as e:
                print(f"LLM Error: {e}")
        return self._get_fallback_insight(anomaly)

    async def _generate_with_llm(self, anomaly: Anomaly) -> str:
        """Call the configured LLM API (Groq / OpenRouter / any OpenAI-compatible endpoint)."""
    ...
    async def generate_correlation_insight(self, correlations: List[Dict[str, Any]]) -> List[str]:
        ...
        if not self.use_llm or not correlations:
