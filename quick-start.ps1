...
# Check prerequisites
Write-Host "`n[OK] Checking prerequisites..." -ForegroundColor Green
...
Write-Host "`n[BOX] Checking Minikube..." -ForegroundColor Cyan
...
Write-Host "`n[BOX] Configuring Docker..." -ForegroundColor Cyan
& minikube docker-env | Invoke-Expression

# Build workload images
Write-Host "`n[DOCKER] Building workload images..." -ForegroundColor Cyan
...
# Create namespaces
Write-Host "`n[CONFIG] Creating Kubernetes namespaces..." -ForegroundColor Cyan
kubectl apply -f k8s/namespaces.yaml

# Apply RBAC
Write-Host "`n[AUTH] Setting up RBAC..." -ForegroundColor Cyan
kubectl apply -f k8s/rbac.yaml

# Apply database service
Write-Host "`n[DB] Setting up database..." -ForegroundColor Cyan
kubectl apply -f k8s/university-data/postgres.yaml

# Apply workload deployments
Write-Host "`n[START] Deploying workloads..." -ForegroundColor Cyan
kubectl apply -f k8s/university-frontend/
kubectl apply -f k8s/university-backend/
kubectl apply -f k8s/university-data/

# Apply Prometheus
Write-Host "`n[MONITORING] Setting up Prometheus..." -ForegroundColor Cyan
kubectl apply -f k8s/prometheus.yaml

# Wait for deployments to be ready
Write-Host "`n[WAIT] Waiting for deployments..." -ForegroundColor Yellow
kubectl wait --for=condition=available --timeout=300s deployment -l app -A

# Start backend
Write-Host "`n[BACKEND] Starting backend (FastAPI)..." -ForegroundColor Cyan
...
if ($backendListening) {
    ...
    $backendProcess = Start-Process python -ArgumentList "-m uvicorn main:app --host 0.0.0.0 --port 8000 --reload" -PassThru
...
# Start frontend
Write-Host "`n[FRONTEND] Starting frontend (React)..." -ForegroundColor Cyan
...
if ($frontendListening) {
    ...
    $frontendProcess = Start-Process npm.cmd -ArgumentList "run start -- --host 0.0.0.0" -PassThru
    Write-Host "  Frontend PID: $($frontendProcess.Id)" -ForegroundColor Green
}
Pop-Location

# Port-forward Prometheus
Write-Host "`n[PORT-FORWARD] Mapping Prometheus to localhost:9090..." -ForegroundColor Cyan
$prometheusForward = Start-Process kubectl -ArgumentList "port-forward svc/prometheus -n monitoring 9090:9090" -WindowStyle Hidden -PassThru
Write-Host "  Prometheus Forward PID: $($prometheusForward.Id)" -ForegroundColor Green

# Print URLs and status
Write-Host "`n[DONE] KubeVision AI Started!" -ForegroundColor Green
...
if ($prometheusForward) {
    Stop-Process -Id $prometheusForward.Id -Force -ErrorAction SilentlyContinue
}
